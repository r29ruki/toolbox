...
sub bbs_list {
	...
	read_log($num,'res') if ($num);
	#read_log($in{res},'res') if ($in{res} > 0);
	
	# 表示タイプ定義
	my ($type,$pg) = get_cookie($cf{cookie_id2});
	$in{type} =~ s/\D//g;
	$in{pg} =~ s/\D//g;
	$in{type} = $in{type} > 0 && $in{type} <= 3 ? $in{type} : $type > 0 ? $type : 1;
	$in{pg} = $in{pg} ne '' ? $in{pg} : $pg ne '' ? $pg : 0;
	set_cookie($cf{cookie_id2},$in{type},$in{pg});
	...
	# テンプレート読込
	my %tmpl = (1 => 'bbs1.html', 2 => 'bbs1.html', 3 => 'bbs3.html');
	...
	foreach (@log) {
		...
		$tmp =~ s/<!-- res -->/$res/g;
		$tmp =~ s|!icon!|<img src="$cf{cmnurl}/face/$ico{$_}" class="image" alt="" />|g;
		$tmp =~ s/!bbs_cgi!/$cf{bbs_cgi}/g;
		$tmp =~ s/!res!/$rno{$_}/g;
...
sub make_pgbtn {
	...
	if ($back >= 0 || $next < $i) {
		...
		while ($z > 0) {
			if ($pg == $y) {
				$flg++;
				push(@pg,qq!<span class="page-numbers current">$x</span>!);
			} else {
				push(@pg,qq!<a class="page-numbers"  href="$cf{bbs_cgi}?pg=$y$stat">$x</a>!);
			}
			$x++;
			$y += $cf{pg_max};
			$z -= $cf{pg_max};
	...
	my $ret = join('',@pg);
	if ($back >= 0) {
		$ret = qq!<a class="prev page-numbers" href="$cf{bbs_cgi}?pg=$back$stat">&laquo; 前へ</a>\n! . $ret;
	}
	if ($next < $i) {
		$ret .= qq!<a class="next page-numbers" href="$cf{bbs_cgi}?pg=$next$stat">&raquo; 次へ</a>\n!;
	}
	
	# 結果を返す
	return $ret ? qq|<ul class="pagenation">$ret</ul>| : '';
}
